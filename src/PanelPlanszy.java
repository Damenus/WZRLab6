...
public class PanelPlanszy extends JPanel{// implements Runnable{
    ...
    String tekst1 = "wersja h", tekst2 = "wersja h";
    ...
    public void paintComponent(Graphics g)  // metoda odrysowująca panel
    {
        ...
        for (int i=0;i<liczba_wierszy;i++)
            for (int j=0;j<liczba_kolumn;j++)
            {
                ...
                if (pola[i][j]==0)  // puste pola
                {
                  //g2.setColor(new Color(220,200,255));
                  //g2.fillRect(j*rozm_pola, i*rozm_pola, rozm_pola, rozm_pola);
                }
                else if (pola[i][j]==1)  // sciany
                {
                  g2.setColor(new Color(110,60,80));
                  g2.fillRect(j*rozm_pola, i*rozm_pola, rozm_pola, rozm_pola);
                }
                else if (pola[i][j]==2) // nagrody
                {
                  g2.setColor(new Color(230,230,40));
        ...
        g2.drawString(tekst1,12,(liczba_wierszy+1)*rozm_pola);
        g2.drawString(tekst2,12,(liczba_wierszy+2)*rozm_pola);
    }
